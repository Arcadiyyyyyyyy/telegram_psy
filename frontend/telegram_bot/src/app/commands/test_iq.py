...
class Conversation(frontend.telegram_bot.src.app.questionary.Conversation):
    ...
    async def command_extension(
        ...
        explainer_message = await context.bot.send_message(
            ...
            "Лучше проходить iq тест со стационарного компьютера или планшета с большим экраном.",
    ...
    async def callback_handler(
        ...
        if (
            ...
            if current_test.get("phase", 0) == 1:
                text += "(!) хотим еще раз обратить внимание, что время на каждый тест ограничено, выполняй так быстро как сможешь.\n\n"  # noqa
